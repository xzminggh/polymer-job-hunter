// 真实岗位数据（支持远程更新 + 本地 bundled 降级）
// 远程数据通过 JobsContext 提供，此文件保留筛选/工具函数
...
/** 获取所有城市列表（去重 + 排序）*/
export const getCities = (jobs: Job[]): string[] => {
  const cities = new Set(jobs.map(j => j.region));
  return Array.from(cities).sort();
};

/** 获取所有岗位类型 */
export const getJobTypes = (jobs: Job[]): string[] => {
  const types = new Set(jobs.map(j => j.jobType));
  return Array.from(types);
};

/** 获取所有学历选项 */
export const getEducationOptions = (jobs: Job[]): string[] => {
  const edus = new Set(jobs.map(j => j.education));
  return Array.from(edus);
};

/** 获取所有单位类型 */
export const getCategories = (jobs: Job[]): string[] => {
...
export const filterJobs = (jobs: Job[], criteria: SearchCriteria): Job[] => {
  return jobs.filter((job: Job) => {
    ...
    if (criteria.publishDateRange && criteria.publishDateRange !== '全部') {
      const days =
        criteria.publishDateRange === '近3天' ? 3 :
        criteria.publishDateRange === '近7天' ? 7 :
        criteria.publishDateRange === '近30天' ? 30 : 0;
      if (days > 0 && job.publishDate && job.publishDate !== '未知') {
        try {
